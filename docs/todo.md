...
- **audit-2026-08-deferred** · **done** (2026-08-01) · P-7/P-8/P-9/§4.4 закрыты, P-4 частично — детали и замеры в CHANGELOG

  - **P-7** — команды объекта теперь берутся из уже разобранного `descriptor` вместо второго `ET.parse`; A/B на ЕРП (18 576 объектов) — 0 расхождений, −32% времени разбора дескрипторов
  - **P-8** — формы разбираются через `ProcessPoolExecutor` (`ConfigurationParser(..., use_process_pool=True)`, по умолчанию включено); байт-паритет с последовательным путём подтверждён, 1.93× на парсинге ЕРП (145 c → 75 c), полная сборка 244.8 c → 169 c
  - **P-4** — `fo_resolver`/`type_name_to_id` больше не блокер сами по себе, но полный стриминг (парсер как генератор объектов) не делали — конфликтует с тем, как P-8 копит форму работы для пула. Сделали более узкое: `_insert_configuration` освобождает тяжёлые поля объекта (`modules`/`forms`/ТЧ/атрибуты/...) сразу после вставки. На ЕРП пик RSS на вставке 6512 МБ → 4373 МБ, к концу вставки 6512 → ~3105 МБ. Пик во время **парсинга** (≥3.8 ГБ) не изменился — это требует стриминга из парсера, оставлено как отдельная задача
  - **P-9** — `_parse_form_module` считает потери в `self.skipped_form_modules`, как `skipped_forms`
  - **§ 4.4** — `PRAGMA synchronous=NORMAL` после `commit()` оказался уже снят в этом же эпике (коммит f6b235f); добавлены `cache_size`/`mmap_size` на read-only соединения сервера
...
- **parser-streaming-pipeline** · **done** (2026-08-01) · Полный стриминг «парсить → вставлять → отпускать»; закрывает остаток P-4
  ...
  - **Что пришлось отложить до конца прохода** (нужен полный справочник объектов): слоты типов (как и раньше) и `fo_form_usage` — `fo_resolver`/`type_name_to_id` теперь копятся в памяти по ходу вставки, а не берутся SELECT-ом между проходами

  - **Верификация:** семантический A/B двух сборок ЕРП (старый код в worktree ↔ новый) — 34 канонических среза, включая 1.9M строк EAV форм, 812k грантов ролей, 423k слотов типов и `fo_form_usage`, **0 расхождений**; счётчики строк совпали по всем таблицам, кроме внутренних сегментных таблиц FTS5 (`code_search_data`/`_idx` — раскладка сегментов зависит от порядка вставки), при этом выдача `MATCH` совпадает построчно и `integrity-check` проходит. `INDEXER_VERSION` не поднимался — формат записываемых данных не изменился (изменился только порядок присвоения суррогатных id)
