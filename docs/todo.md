...
- **audit-2026-08-deferred** · `idea` · Отложено под метрики (из аудита 2026-08)
  ...
  - **P-8 · Формы — 58% времени парсинга** (84.7 c из 146 c), читаются последовательно, файлы независимы, тяжёлая часть в C → `ProcessPoolExecutor` даёт близкий к линейному выигрыш и снимает потолок памяти P-4

  - **P-4 (уточнение) · Дерево объектов в памяти — ≥3.8 ГБ RSS** на ЕРП поверх `cache_size=-256000`. Блокирует стриминг только порядок построения `fo_resolver`/`type_name_to_id`

  - **P-9 · Голый `except:` в `_parse_form_module`** — потеря модуля формы нигде не считается (в отличие от `skipped_forms`)

  - **§ 4.4 · Сервер:** read-only соединения без `cache_size`/`mmap_size` (кэш 2 МБ на файл 3.3 ГБ); `PRAGMA synchronous=NORMAL` после `commit()` перед закрытием — мёртвый код
