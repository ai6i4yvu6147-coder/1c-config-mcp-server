...
Масштаб (реальный корпус АСБ/Альфа/Трансгаз/Фитэра): **~22 264** MXL-макета — на порядок больше,
...
(FTS) как модуль `module_type='MxlText'`; владелец строки — объект макета. Тогда `search_code`
...
## Политика индексации по виду корня (2026-07-19)

Реальный корпус вскрыл цену среза 1 на **чтении**: макетов на порядок больше, чем модулей
(Альфа ЕРП — **15 445** тел `Template.xml`), и движок читал каждое тело — полная пересборка
крупной конфигурации замедлилась в разы. При этом в разметке макета почти нет аналитической
ценности (метки/форматирование; вся логика — в коде/СКД). Поэтому индексация макетов:

- **выключена по умолчанию для конфигураций и расширений** — флаг
  `ConfigurationParser.index_spreadsheet_templates` = `False`, `_parse_spreadsheet_templates`
  делает ранний возврат `[]` (тела не читаются, дескрипторы `Templates/` не парсятся повторно —
  ДКС-обход уже парсит их один раз);
- **включена для внешних отчётов/обработок** — `parse()` выставляет флаг `True` по виду корня
  (`ExternalReport`/`ExternalDataProcessor`), где макет — суть объекта, а объектов мало.

**СКД под флаг не попадает** — текст запроса ценен всегда. Заодно у внешних отчётов/обработок
теперь читается и СКД: `_parse_external_root` обходит `Templates/` off-disk (раньше
`_adapt_object_descriptor` игнорировал `Template`-ссылки вовсе → чистый внешний СКД-отчёт давал
тонкий индекс). `INDEXER_VERSION` 18 → **19** (контент БД меняется: конфигурации теряют строки
`MxlText`, внешние получают `DcsQuery`/`MxlText`/`dcs_schema`). Тесты — `tests/test_mxl_macet_policy.py`.
