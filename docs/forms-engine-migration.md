...
- **Шаг 3 — очистка** (позже): снять legacy-дублирование форматного чтения там, где оно теперь
  живёт только в движке; `INDEXER_VERSION` поднимать только при смене формы EAV-данных
  (не менялась — 17).
