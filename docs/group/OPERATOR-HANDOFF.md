...
Human-tier — русский OK. В хаб-модели (canon 2.6.0) синхронизация протокола идёт через `GROUP-HUB.md` — оператор **не копирует пакеты** между репозиториями. Sub читает контракт напрямую из Head `docs/group/shared/` at `head.path`. За оператором остаются только вещи вне контекста агента: учётные данные и деплой.
