...
3. **Развилка с фолбэком.** `parse_rights_xml` = «библиотека, иначе legacy»:
   `parse_rights_xml_via_library` (движок) с падением в `parse_rights_xml_legacy`
   (сохранённый ET-парсер) на любой ошибке/отсутствии библиотеки. Роли **не зависят** от
   импортируемости библиотеки (в отличие от внешних отчётов/обработок).
...
- **A/B-верификация:** `parse_rights_xml_legacy` ↔ `parse_rights_xml_via_library` на
  **всех 3010** `Rights.xml` (АСБ/Планета/Трансгаз/Фитэра): **946 091** грант, **11 993**
